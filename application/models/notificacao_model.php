...
Class Notificacao_model extends CI_Model{
	...
	public function listarNotificacoes($estado,$cidade,$cnpjRaiz,$cnpj){
...
public function listarNotificacoesByEstado($estado){
		...
		$sql = "SELECT em.nome,em.email,em.cargo,enc.*,DATE_FORMAT(enc.data_envio,'%d/%m/%Y %H:%i:%s') as data_envio_br,ienc.arquivo FROM notificacoes_encaminhamento enc  left join email em on enc.id_email = em.id left join notificacoes_enc_arquivos ienc on enc.id = ienc.id_notificacao_enc where enc.id_notificacao = ? order by enc.id desc";  
	...
	public function listarNotificacaoById($id){
		$sql = "select i.*,c.cnpj,cr.cnpj_raiz,DATE_FORMAT(i.data_ciencia,'%d/%m/%Y') as data_ciencia_br,DATE_FORMAT(i.prazo,'%d/%m/%Y') as prazo_br,ie.numero as num_ie,im.numero as num_im,c.id_uf,c.id_municipio,cr.id as id_cnpj_raiz,c.ID as id_cnpj  from notificacoes i join cnpj c on i.id_cnpj = c.id join cnpj_raiz cr on c.id_cnpj_raiz = cr.id left join inscricao ie on ie.id = i.id_ie and ie.tipo = 1  left join inscricao im on im.id = i.id_im and im.tipo = 2 where i.id = ?"; 
		$query = $this->db->query($sql, array($id));
		//print_r($this->db->last_query());exit;
		$array = $query->result(); //array of arrays
		return($array);
	...
	public function contaNotificacaoByUf($uf){
		if($uf == '0'){
			$sql = "select count(*) as total from notificacoes i join cnpj c on i.id_cnpj = c.id left join estados e on e.id = c.id_uf "; 
			$query = $this->db->query($sql, array());
		}else{
			$sql = "select count(*) as total from notificacoes i join cnpj c on i.id_cnpj = c.id   left join estados e on e.id = c.id_uf where e.uf = ?"; 
