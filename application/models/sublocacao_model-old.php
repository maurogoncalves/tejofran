...
Class Sublocacao_model extends CI_Model{
  ...
  function listarSubLocacao($id_contratante,$_limit = 30, $_start = 0 ){
      $sql = "select 
		    ...
		    ORDER BY `em`.`nome_fantasia` limit $_start,$_limit";
 ...
 function listarLojasComLocacao($idContratante){
      $sql = "SELECT sublocacoes.id,emitente.nome_fantasia
 ...
 function listarSubLocacaoById($id_contratante,$id ){
      $sql = "select sublocacoes.id as idsub, em.nome_fantasia as locador, 
				...
				data_fim_vigencia,
				DATE_FORMAT(sublocacoes.data_ini_vigencia,'%d/%m/%Y') as data_ini_vigencia_br,
				DATE_FORMAT(sublocacoes.data_fim_vigencia,'%d/%m/%Y') as data_fim_vigencia_br,metragem,atividade_sublocada
				 from sublocacoes 
