...
$sql = "SELECT *, `fgts`.`ativo` as status_insc, `fgts`.`id` as id_cnd, `loja`.`ins_cnd_mob` as ins_cnd_mob, `loja`.`id` as id_loja, `regional`.`descricao` as desc_regional, `bandeira`.`descricao_bandeira`, DATE_FORMAT(data_emissao, '%d/%m/%Y') as data_emissao_br 
FROM (`fgts`) LEFT JOIN `loja` ON `fgts`.`id_loja` = `loja`.`id` 
LEFT JOIN `imovel` ON `imovel`.`id` = `loja`.`id_imovel` 
LEFT JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` 
LEFT JOIN `regional` ON `regional`.`id` = `loja`.`regional` LEFT JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira` WHERE `fgts`.`id` = ?";
$query = $this->db->query($sql, array($id));
