...
function listarInscricaoById($id){
 ...
 function contaVigente($idContratante,$ano,$tipo,$statusVigente){
	 
	 $data = date("Y-m-d");
	 if($statusVigente == 2){
		 
		 $sql = "SELECT count(*) as total FROM (`cnd_imobiliaria`)  JOIN `iptu` ON `cnd_imobiliaria`.`id_iptu` = `iptu`.`id`  WHERE `cnd_imobiliaria`.`id_contratante` = ?
		 AND `cnd_imobiliaria`.`possui_cnd` = ?  and iptu.ano_ref = ? and data_vencto < ?";
		$query = $this->db->query($sql, array($idContratante,$tipo,$ano,$data));	
		 
	 }else{
		 
		 $sql = "SELECT count(*) as total FROM (`cnd_imobiliaria`)  JOIN `iptu` ON `cnd_imobiliaria`.`id_iptu` = `iptu`.`id` 
		WHERE `cnd_imobiliaria`.`id_contratante` = ?  AND `cnd_imobiliaria`.`possui_cnd` = ? and iptu.ano_ref = ? and data_vencto > ?";
		$query = $this->db->query($sql, array($idContratante,$tipo,$ano,$data));	
	 }
