...
//print_r($this->db->last_query());exit;
   if($query -> num_rows() <> 0){
	...
	public function contaProtestoByData($data,$tipo,$tipoOcorrencia){
		...
		if($tipo == 1){
			$sql = "select count(*) as total from protesto p where p.data_protesto = ? and tipo_ocorrencia = ?"; 
		}else{
			$sql = "select count(*) as total from protesto p where p.data_baixa_protesto = ? and tipo_ocorrencia = ?"; 
		}
		
		$query = $this->db->query($sql, array($data,$tipoOcorrencia));
		...
		public function contaProtestoByDataAux($data,$tipo,$tipoOcorrencia){
