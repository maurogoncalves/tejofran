...
class Home extends CI_Controller {
 ...
 function dashboard(){	
	...
	if(empty($_POST)){
		...
		if($tipOcorrencia == 0){
			$data['tipoOcorrencia'] =  $tipoOcorrencia = 1;
			$data['descTipo'] = 'Protesto';
	...
	while ($endDate >= $startDate) {
		...
		$dataEntrada = $this->protesto_model->contaProtestoByData($dataMov,1,$tipoOcorrencia); //entrada protesto
		$dataSaida = $this->protesto_model->contaProtestoByData($dataMov,2,$tipoOcorrencia); //saida protesto
		
		$dataMovArr = explode("-",$dataMov);
		
		$graficoDatasProtesto[$i]['data']= '01-'.$dataMovArr[1].'-'.$dataMovArr[0];
		$graficoDatasProtesto[$i]['entrada']=$dataEntrada[0]->total;
		$graficoDatasProtesto[$i]['saida']=$dataSaida[0]->total;
		$i++;
	}
	$graficoDatasProtesto = array_reverse($graficoDatasProtesto);
