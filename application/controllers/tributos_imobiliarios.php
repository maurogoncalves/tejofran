...
class Tributos_imobiliarios extends CI_Controller {
 ...
 if(!$_SESSION['login']){
	...
	$regionais = $this->tributos_imobiliarios_model->regionais();	
	$regionalSim = array();
	$regionalNao = array();
	$regionalPend = array();
	$regionalTodos = array();
	...
	foreach($regionais as $reg){
		$result = $this->tributos_imobiliarios_model->contaCndRegional($idContratante,$ano,1,$reg->id);
	...
	$i=1;
	foreach($regionais as $reg){
		$result = $this->tributos_imobiliarios_model->contaCndRegional($idContratante,$ano,2,$reg->id);
	...
	$i=1;
	foreach($regionais as $reg){
		$result = $this->tributos_imobiliarios_model->contaCndRegional($idContratante,$ano,3,$reg->id);
	...
	$i=1;
	foreach($regionais as $reg){
		$result = $this->tributos_imobiliarios_model->contaCndRegional($idContratante,$ano,4,$reg->id);
	...
	//seleciona todos
	foreach($regionais as $reg){
		$result = $this->tributos_imobiliarios_model->contaCndRegional($idContratante,$ano,0,$reg->id);
	...
	$data['modulo'] = 'tributos_imobiliarios';
	$data['nome_modulo'] = 'Tributos Imobiliários';
	$data['iptus'] = $this->tributos_imobiliarios_model->contaCnd($idContratante,$ano);
 ...
 if(!$_SESSION['login']){
	...
	$data['emitente'] = $this->tributos_imobiliarios_model->listarEmitenteById($id_loja);
 ...
 function enviar(){		
	...
	if (!$this->upload->do_upload($field_name)){			
		...
		$dados = array('arquivo_cnd' => 'tributos_imobiliarios-'.$id.'.'.$extensao);							
		$this->tributos_imobiliarios_model->atualizar($dados,$id);		
		$data = array('upload_data' => $this->upload->data($field_name));		
		$_SESSION['mensagemIptu'] =  UPLOAD;
		echo'1';
...
//redirect('/iptu/listar', 'refresh');		 
 ...
 function enviarOld(){
		...
		$dadosImovel = $this->tributos_imobiliarios_model->listarCidadeEstadoById($id);
 ...
 function enviar_pend(){		
	...
	if (!$this->upload->do_upload($field_name)){			
		...
		$dados = array(
			'arquivo_pendencias' => 'tributos_imobiliarios-'.$id.'.'.$extensao
			);	
...
//redirect('/iptu/listar', 'refresh');		 
 ...
 function upload_cnd(){
  ...
  $data['dataEmissao'] = $this->tributos_imobiliarios_model->listarTodasDataEmissao($id,'mob');		

  $dados = $this->tributos_imobiliarios_model->listarInscricaoById($id);
	$data['imovel'] = $dados;
 ...
 function ver(){
  ...
  $data['imovel'] = $this->tributos_imobiliarios_model->listarInscricaoById($id);
 ...
 function cadastrar_cnd_mob(){
   ...
   if(!$_SESSION['login']){
	...
	$id = $this->tributos_imobiliarios_model->add($dados);
	...
	if($id) {
		...
		$this->tributos_imobiliarios_model->addDataEmissao($dadosDataEmissao,$id,'mob');
 ...
 function inativar(){
	...
	if($podeExcluir[0]['total'] == 1){		
		$this->tributos_imobiliarios_model->excluirFisicamente($id);
		$_SESSION['mensagemImovel'] =  CADASTRO_APAGADO;		
	}else{	
		$dados = array('ativo' => 1);
		if($this->tributos_imobiliarios_model->atualizar($dados,$id)) {
			$_SESSION['mensagemCNDMOB'] =  CADASTRO_INATIVO;	
		}else{	
			$_SESSION['mensagemCNDMOB'] =  ERRO;	
		}
 ...
 function atualizar_cnd(){
	...
	if($possui_cnd == 1){
		...
		if(empty($data_emissao)){
			...
			redirect('/tributos_imobiliarios/'.$modulo);	
			exit;
		}
		$arrDataEmissao = explode("/",$data_emissao);	
		$dataEmissao = $arrDataEmissao[2].'-'.$arrDataEmissao[1].'-'.$arrDataEmissao[0];
		
		$dadosDataDb = $this->tributos_imobiliarios_model->listarDataEmissao($id,'mob');
		...
		if($isArray == 1){
			...
			if($dadosDataDb[0]->data_emissao <> $dataEmissao ){
				$this->tributos_imobiliarios_model->addDataEmissao($dadosDataEmissao);
			}
	...
	$dadosAtuais = $this->tributos_imobiliarios_model->listarCNDById($id);
...
function export_mun(){	  
  if(!$_SESSION['login']){
	...
	$session_data = $_SESSION['login'];
	$idContratante = $_SESSION['cliente'] ;	
	$result = $this->tributos_imobiliarios_model->listarIptuCsvByCidade($id,$tipo);
	$this->csv($result);		
 }
 ...
 function export_est(){	  
  if(!$_SESSION['login']){
	...
	$session_data = $_SESSION['login'];
	$idContratante = $_SESSION['cliente'] ;
	$result = $this->tributos_imobiliarios_model->listarIptuCsvByEstado($id,$tipo);
	$this->csv($result);
 }
  ...
  if(!$_SESSION['login']){
	...
	if($id == 0){
		...
		$result = $this->tributos_imobiliarios_model->listarIptuCsv($idContratante,$tipo);
	}else{

		$result = $this->tributos_imobiliarios_model->listarIptuCsvById($id,$tipo);
	}
 ...
 function buscaCidade(){	
	...
	$retorno ='';
	$result = $this->tributos_imobiliarios_model->listarCidadeByEstado($idContratante,$id,$tipo);
 ...
 function buscaTodasCidades(){	
	...
	$retorno ='';
	$result = $this->tributos_imobiliarios_model->listarCidade($idContratante,$tipo);
 ...
 function buscaInscricaoByEstado(){	
	...
	$retorno ='';
	$result = $this->tributos_imobiliarios_model->listarImovelByEstado($id,$tipo);
 ...
 function buscaLoja(){	
	...
	$retorno ='';
	$result = $this->tributos_imobiliarios_model->listarImovelByCidade($id,$tipo);
 ...
 function busca(){	
	...
	$retorno ='';
	$result = $this->tributos_imobiliarios_model->listarLojaByImovel($id,$tipo);
 ...
 function buscaPendentes(){	
	...
	$retorno ='';
	$result = $this->tributos_imobiliarios_model->listarImovelPendente();
  ...
  function buscaEstado(){	
	...
	$retorno ='';
	$result = $this->tributos_imobiliarios_model->listarImovelByUf($id,$tipo);
 ...
 function buscaMunicipio(){	
	...
	$retorno ='';
	$result = $this->tributos_imobiliarios_model->listarImovelByMunicipio($id,$tipo);
 ...
 if(!$_SESSION['login']){
	...
	$idContratante = $_SESSION['cliente'] ;				
	$data['perfil'] = $session_data['perfil'];
	$data['lojas'] = $this->tributos_imobiliarios_model->listarLoja($idContratante,$tipo);	
	$data['estados'] = $this->tributos_imobiliarios_model->listarEstado($idContratante,$tipo);	
	$data['cidades'] = $this->tributos_imobiliarios_model->listarCidade($idContratante,$tipo);
	$session_data = $_SESSION['login'];
	...
	if(empty($_POST)){
		...
		$data['estadoBD'] = 0;	
		$data['idCNDMBD'] = 0;
		$result = $this->tributos_imobiliarios_model->listarCndTipo($idContratante,$tipo);
		...
		if($idImovelListar == '0'){	
			if($municipioListar <> '0'){
				$result =  $this->tributos_imobiliarios_model->listarCndTipoMun($idContratante,$municipioListar,$tipo);
			}else{
				$result =  $this->tributos_imobiliarios_model->listarCndTipoEst($idContratante,$estadoListar,$tipo);
			}
		}else{	
			$result = $this->tributos_imobiliarios_model->listarCndTipoId($idContratante,$idImovelListar,$tipo);
		}
	}
 ...
 if(!$_SESSION['login']){
	...
	$result =  $this->tributos_imobiliarios_model->listarCndTipoReg($idContratante,$regional,$tipo);
...
function listarPorTipoNao(){	
 ...
 if(!$_SESSION['login']){
	...
	$data['lojas'] = $this->tributos_imobiliarios_model->listarLoja($idContratante,'X');
	
	$data['estados'] = $this->tributos_imobiliarios_model->listarEstado($idContratante,'X');
	
	$data['cidades'] = $this->tributos_imobiliarios_model->listarCidade($idContratante,'X');
	...
	$result = $this->tributos_imobiliarios_model->listarCnd($idContratante,numRegister4PagePaginate(), $page,$cidade,$estado);
	
	
	
	$total = $this->tributos_imobiliarios_model->somarTodos($idContratante,$cidade,$estado);
 ...
 if(!$_SESSION['login']){
	...
	$data['lojas'] = $this->tributos_imobiliarios_model->listarLoja($idContratante);
	
	$data['estados'] = $this->tributos_imobiliarios_model->listarEstado($idContratante);
	
	$data['cidades'] = $this->tributos_imobiliarios_model->listarCidade($idContratante);
