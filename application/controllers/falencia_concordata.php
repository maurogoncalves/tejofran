...
class Falencia_concordata extends CI_Controller {
 ...
 if(!$_SESSION['login']){
	...
	$regionais = $this->falencia_concordata_model->regionais();	
	$regionalSim = array();
	$regionalNao = array();
	$regionalPend = array();
	$regionalTodos = array();
	...
	foreach($regionais as $reg){
		$result = $this->falencia_concordata_model->contaCndRegional($idContratante,$ano,1,$reg->id);
	...
	$i=1;
	foreach($regionais as $reg){
		$result = $this->falencia_concordata_model->contaCndRegional($idContratante,$ano,2,$reg->id);
	...
	$i=1;
	foreach($regionais as $reg){
		$result = $this->falencia_concordata_model->contaCndRegional($idContratante,$ano,3,$reg->id);
	...
	$i=1;
	foreach($regionais as $reg){
		$result = $this->falencia_concordata_model->contaCndRegional($idContratante,$ano,4,$reg->id);
	...
	//seleciona todos
	foreach($regionais as $reg){
		$result = $this->falencia_concordata_model->contaCndRegional($idContratante,$ano,0,$reg->id);
	...
	$data['modulo'] = 'falencia_concordata';
	$data['nome_modulo'] = 'Falência Concordata';
	$data['iptus'] = $this->falencia_concordata_model->contaCnd($idContratante,$ano);
 ...
 if(!$_SESSION['login']){
	...
	$data['emitente'] = $this->falencia_concordata_model->listarEmitenteById($id_loja);
 ...
 function enviar(){		
	...
	if (!$this->upload->do_upload($field_name)){			
		...
		$dados = array('arquivo_cnd' => 'falencia_concordata-'.$id.'.'.$extensao);							
		$this->falencia_concordata_model->atualizar($dados,$id);		
		$data = array('upload_data' => $this->upload->data($field_name));		
		$_SESSION['mensagemIptu'] =  UPLOAD;
		echo'1';
...
//redirect('/iptu/listar', 'refresh');		 
 ...
 function enviarOld(){
		...
		$dadosImovel = $this->falencia_concordata_model->listarCidadeEstadoById($id);
 ...
 function enviar_pend(){		
	...
	if (!$this->upload->do_upload($field_name)){			
		...
		$dados = array(
			'arquivo_pendencias' => 'falencia_concordata-'.$id.'.'.$extensao
			);	
...
//redirect('/iptu/listar', 'refresh');		 
 ...
 function upload_cnd(){
  ...
  $data['dataEmissao'] = $this->falencia_concordata_model->listarTodasDataEmissao($id,'mob');		

  $dados = $this->falencia_concordata_model->listarInscricaoById($id);
	$data['imovel'] = $dados;
 ...
 function ver(){
  ...
  $data['imovel'] = $this->falencia_concordata_model->listarInscricaoById($id);
 ...
 function cadastrar_cnd_mob(){
   ...
   if(!$_SESSION['login']){
	...
	$id = $this->falencia_concordata_model->add($dados);
	...
	if($id) {
		...
		$this->falencia_concordata_model->addDataEmissao($dadosDataEmissao,$id,'mob');
 ...
 function inativar(){
	...
	if($podeExcluir[0]['total'] == 1){		
		$this->falencia_concordata_model->excluirFisicamente($id);
		$_SESSION['mensagemImovel'] =  CADASTRO_APAGADO;		
	}else{	
		$dados = array('ativo' => 1);
		if($this->falencia_concordata_model->atualizar($dados,$id)) {
			$_SESSION['mensagemCNDMOB'] =  CADASTRO_INATIVO;	
		}else{	
			$_SESSION['mensagemCNDMOB'] =  ERRO;	
		}
 ...
 function atualizar_cnd(){
	...
	if($possui_cnd == 1){
		...
		if(empty($data_emissao)){
			...
			redirect('/falencia_concordata/'.$modulo);	
			exit;
		}
		$arrDataEmissao = explode("/",$data_emissao);	
		$dataEmissao = $arrDataEmissao[2].'-'.$arrDataEmissao[1].'-'.$arrDataEmissao[0];
		
		$dadosDataDb = $this->falencia_concordata_model->listarDataEmissao($id,'mob');
		...
		if($isArray == 1){
			...
			if($dadosDataDb[0]->data_emissao <> $dataEmissao ){
				$this->falencia_concordata_model->addDataEmissao($dadosDataEmissao);
			}
	...
	$dadosAtuais = $this->falencia_concordata_model->listarCNDById($id);
...
function export_mun(){	  
  if(!$_SESSION['login']){
	...
	$session_data = $_SESSION['login'];
	$idContratante = $_SESSION['cliente'] ;	
	$result = $this->falencia_concordata_model->listarIptuCsvByCidade($id,$tipo);
	$this->csv($result);		
 }
 ...
 function export_est(){	  
  if(!$_SESSION['login']){
	...
	$session_data = $_SESSION['login'];
	$idContratante = $_SESSION['cliente'] ;
	$result = $this->falencia_concordata_model->listarIptuCsvByEstado($id,$tipo);
	$this->csv($result);
 }
  ...
  if(!$_SESSION['login']){
	...
	if($id == 0){
		...
		$result = $this->falencia_concordata_model->listarIptuCsv($idContratante,$tipo);
	}else{

		$result = $this->falencia_concordata_model->listarIptuCsvById($id,$tipo);
	}
 ...
 function buscaCidade(){	
	...
	$retorno ='';
	$result = $this->falencia_concordata_model->listarCidadeByEstado($idContratante,$id,$tipo);
 ...
 function buscaTodasCidades(){	
	...
	$retorno ='';
	$result = $this->falencia_concordata_model->listarCidade($idContratante,$tipo);
 ...
 function buscaInscricaoByEstado(){	
	...
	$retorno ='';
	$result = $this->falencia_concordata_model->listarImovelByEstado($id,$tipo);
 ...
 function buscaLoja(){	
	...
	$retorno ='';
	$result = $this->falencia_concordata_model->listarImovelByCidade($id,$tipo);
 ...
 function busca(){	
	...
	$retorno ='';
	$result = $this->falencia_concordata_model->listarLojaByImovel($id,$tipo);
 ...
 function buscaPendentes(){	
	...
	$retorno ='';
	$result = $this->falencia_concordata_model->listarImovelPendente();
  ...
  function buscaEstado(){	
	...
	$retorno ='';
	$result = $this->falencia_concordata_model->listarImovelByUf($id,$tipo);
 ...
 function buscaMunicipio(){	
	...
	$retorno ='';
	$result = $this->falencia_concordata_model->listarImovelByMunicipio($id,$tipo);
 ...
 if(!$_SESSION['login']){
	...
	$idContratante = $_SESSION['cliente'] ;				
	$data['perfil'] = $session_data['perfil'];
	$data['lojas'] = $this->falencia_concordata_model->listarLoja($idContratante,$tipo);	
	$data['estados'] = $this->falencia_concordata_model->listarEstado($idContratante,$tipo);	
	$data['cidades'] = $this->falencia_concordata_model->listarCidade($idContratante,$tipo);
	$session_data = $_SESSION['login'];
	...
	if(empty($_POST)){
		...
		$data['estadoBD'] = 0;	
		$data['idCNDMBD'] = 0;
		$result = $this->falencia_concordata_model->listarCndTipo($idContratante,$tipo);
		...
		if($idImovelListar == '0'){	
			if($municipioListar <> '0'){
				$result =  $this->falencia_concordata_model->listarCndTipoMun($idContratante,$municipioListar,$tipo);
			}else{
				$result =  $this->falencia_concordata_model->listarCndTipoEst($idContratante,$estadoListar,$tipo);
			}
		}else{	
			$result = $this->falencia_concordata_model->listarCndTipoId($idContratante,$idImovelListar,$tipo);
		}
	}
 ...
 if(!$_SESSION['login']){
	...
	$result =  $this->falencia_concordata_model->listarCndTipoReg($idContratante,$regional,$tipo);
...
function listarPorTipoNao(){	
 ...
 if(!$_SESSION['login']){
	...
	$data['lojas'] = $this->falencia_concordata_model->listarLoja($idContratante,'X');
	
	$data['estados'] = $this->falencia_concordata_model->listarEstado($idContratante,'X');
	
	$data['cidades'] = $this->falencia_concordata_model->listarCidade($idContratante,'X');
	...
	$result = $this->falencia_concordata_model->listarCnd($idContratante,numRegister4PagePaginate(), $page,$cidade,$estado);
	
	
	
	$total = $this->falencia_concordata_model->somarTodos($idContratante,$cidade,$estado);
 ...
 if(!$_SESSION['login']){
	...
	$data['lojas'] = $this->falencia_concordata_model->listarLoja($idContratante);
	
	$data['estados'] = $this->falencia_concordata_model->listarEstado($idContratante);
	
	$data['cidades'] = $this->falencia_concordata_model->listarCidade($idContratante);
