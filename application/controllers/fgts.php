...
class Fgts extends CI_Controller {
 ...
 if(!$_SESSION['login']){
	...
	$regionais = $this->fgts_model->regionais();	
	$regionalSim = array();
	$regionalNao = array();
	$regionalPend = array();
	$regionalTodos = array();
	...
	foreach($regionais as $reg){
		$result = $this->fgts_model->contaCndRegional($idContratante,$ano,1,$reg->id);
	...
	$i=1;
	foreach($regionais as $reg){
		$result = $this->fgts_model->contaCndRegional($idContratante,$ano,2,$reg->id);
	...
	$i=1;
	foreach($regionais as $reg){
		$result = $this->fgts_model->contaCndRegional($idContratante,$ano,3,$reg->id);
	...
	$i=1;
	foreach($regionais as $reg){
		$result = $this->fgts_model->contaCndRegional($idContratante,$ano,4,$reg->id);
	...
	//seleciona todos
	foreach($regionais as $reg){
		$result = $this->fgts_model->contaCndRegional($idContratante,$ano,0,$reg->id);
	...
	$data['modulo'] = 'fgts';
	$data['nome_modulo'] = 'Fgts';
	$data['iptus'] = $this->fgts_model->contaCnd($idContratante,$ano);
 ...
 if(!$_SESSION['login']){
	...
	$data['emitente'] = $this->fgts_model->listarEmitenteById($id_loja);
 ...
 function enviar(){		
	...
	if (!$this->upload->do_upload($field_name)){			
		...
		$dados = array('arquivo_cnd' => 'fgts-'.$id.'.'.$extensao);							
		$this->fgts_model->atualizar($dados,$id);		
		$data = array('upload_data' => $this->upload->data($field_name));		
		$_SESSION['mensagemIptu'] =  UPLOAD;
		echo'1';
...
//redirect('/iptu/listar', 'refresh');		 
 ...
 function enviarOld(){
		...
		$dadosImovel = $this->fgts_model->listarCidadeEstadoById($id);
 ...
 function enviar_pend(){		
	...
	if (!$this->upload->do_upload($field_name)){			
		...
		$dados = array(
			'arquivo_pendencias' => 'fgts-'.$id.'.'.$extensao
			);	
...
//redirect('/iptu/listar', 'refresh');		 
 ...
 function upload_cnd(){
  ...
  $data['dataEmissao'] = $this->fgts_model->listarTodasDataEmissao($id,'mob');		

  $dados = $this->fgts_model->listarInscricaoById($id);
	$data['imovel'] = $dados;
 ...
 function ver(){
  ...
  $data['imovel'] = $this->fgts_model->listarInscricaoById($id);
 ...
 function cadastrar_cnd_mob(){
   ...
   if(!$_SESSION['login']){
	...
	if($id) {
		...
		$this->fgts_model->addDataEmissao($dadosDataEmissao,$id,'mob');
 ...
 function inativar(){
	...
	if($podeExcluir[0]['total'] == 1){		
		$this->fgts_model->excluirFisicamente($id);
		$_SESSION['mensagemImovel'] =  CADASTRO_APAGADO;		
	}else{	
		$dados = array('ativo' => 1);
		if($this->fgts_model->atualizar($dados,$id)) {
			$_SESSION['mensagemCNDMOB'] =  CADASTRO_INATIVO;	
		}else{	
			$_SESSION['mensagemCNDMOB'] =  ERRO;	
		}
 ...
 function atualizar_cnd(){
	...
	if($possui_cnd == 1){
		...
		if(empty($data_emissao)){
			...
			redirect('/fgts/'.$modulo);	
			exit;
		}
		$arrDataEmissao = explode("/",$data_emissao);	
		$dataEmissao = $arrDataEmissao[2].'-'.$arrDataEmissao[1].'-'.$arrDataEmissao[0];
		
		$dadosDataDb = $this->fgts_model->listarDataEmissao($id,'mob');
		...
		if($isArray == 1){
			...
			if($dadosDataDb[0]->data_emissao <> $dataEmissao ){
				$this->fgts_model->addDataEmissao($dadosDataEmissao);
			}
	...
	$dadosAtuais = $this->fgts_model->listarCNDById($id);
...
function export_mun(){	  
  if(!$_SESSION['login']){
	...
	$session_data = $_SESSION['login'];
	$idContratante = $_SESSION['cliente'] ;	
	$result = $this->fgts_model->listarIptuCsvByCidade($id,$tipo);
	$this->csv($result);		
 }
 ...
 function export_est(){	  
  if(!$_SESSION['login']){
	...
	$session_data = $_SESSION['login'];
	$idContratante = $_SESSION['cliente'] ;
	$result = $this->fgts_model->listarIptuCsvByEstado($id,$tipo);
	$this->csv($result);
 }
  ...
  if(!$_SESSION['login']){
	...
	if($id == 0){
		...
		$result = $this->fgts_model->listarIptuCsv($idContratante,$tipo);
	}else{

		$result = $this->fgts_model->listarIptuCsvById($id,$tipo);
	}
 ...
 function buscaCidade(){	
	...
	$retorno ='';
	$result = $this->fgts_model->listarCidadeByEstado($idContratante,$id,$tipo);
 ...
 function buscaTodasCidades(){	
	...
	$retorno ='';
	$result = $this->fgts_model->listarCidade($idContratante,$tipo);
 ...
 function buscaInscricaoByEstado(){	
	...
	$retorno ='';
	$result = $this->fgts_model->listarImovelByEstado($id,$tipo);
 ...
 function buscaLoja(){	
	...
	$retorno ='';
	$result = $this->fgts_model->listarImovelByCidade($id,$tipo);
 ...
 function busca(){	
	...
	$retorno ='';
	$result = $this->fgts_model->listarLojaByImovel($id,$tipo);
 ...
 function buscaPendentes(){	
	...
	$retorno ='';
	$result = $this->fgts_model->listarImovelPendente();
  ...
  function buscaEstado(){	
	...
	$retorno ='';
	$result = $this->fgts_model->listarImovelByUf($id,$tipo);
 ...
 function buscaMunicipio(){	
	...
	$retorno ='';
	$result = $this->fgts_model->listarImovelByMunicipio($id,$tipo);
 ...
 if(!$_SESSION['login']){
	...
	$idContratante = $_SESSION['cliente'] ;				
	$data['perfil'] = $session_data['perfil'];
	$data['lojas'] = $this->fgts_model->listarLoja($idContratante,$tipo);	
	$data['estados'] = $this->fgts_model->listarEstado($idContratante,$tipo);	
	$data['cidades'] = $this->fgts_model->listarCidade($idContratante,$tipo);
	$session_data = $_SESSION['login'];
	...
	if(empty($_POST)){
		...
		$data['estadoBD'] = 0;	
		$data['idCNDMBD'] = 0;
		$result = $this->fgts_model->listarCndTipo($idContratante,$tipo);
		...
		if($idImovelListar == '0'){	
			if($municipioListar <> '0'){
				$result =  $this->fgts_model->listarCndTipoMun($idContratante,$municipioListar,$tipo);
			}else{
				$result =  $this->fgts_model->listarCndTipoEst($idContratante,$estadoListar,$tipo);
			}
		}else{	
			$result = $this->fgts_model->listarCndTipoId($idContratante,$idImovelListar,$tipo);
		}
	}
 ...
 if(!$_SESSION['login']){
	...
	$result =  $this->fgts_model->listarCndTipoReg($idContratante,$regional,$tipo);
...
function listarPorTipoNao(){	
 ...
 if(!$_SESSION['login']){
	...
	$data['lojas'] = $this->fgts_model->listarLoja($idContratante,'X');
	
	$data['estados'] = $this->fgts_model->listarEstado($idContratante,'X');
	
	$data['cidades'] = $this->fgts_model->listarCidade($idContratante,'X');
	...
	$result = $this->fgts_model->listarCnd($idContratante,numRegister4PagePaginate(), $page,$cidade,$estado);
	
	
	
	$total = $this->fgts_model->somarTodos($idContratante,$cidade,$estado);
 ...
 if(!$_SESSION['login']){
	...
	$data['lojas'] = $this->fgts_model->listarLoja($idContratante);
	
	$data['estados'] = $this->fgts_model->listarEstado($idContratante);
	
	$data['cidades'] = $this->fgts_model->listarCidade($idContratante);
